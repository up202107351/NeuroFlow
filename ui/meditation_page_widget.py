...
class MeditationPageWidget(QtWidgets.QWidget):
    ...
    def _handle_signal_quality_update(self, quality_data):
        """Process signal quality updates from backend"""
        if not self.video_player_window or not hasattr(self.video_player_window, 'signal_quality_widget'):
            return
            
        if not self.video_player_window.signal_quality_widget:
            return
        ...
        try:
            ...
            if "quality_metrics" in quality_data:
                ...
                # Create a simple metrics object or update widget directly
                quality_info = {
                    'movement_score': metrics.get('movement_score', 0.5),
                    'band_power_score': metrics.get('band_power_score', 0.5),
                    'electrode_contact_score': metrics.get('electrode_contact_score', 0.5),
                ...
                self.video_player_window.signal_quality_widget.update_metrics(quality_info)
            
            # Handle accelerometer data if present
            if "accelerometer" in quality_data:
                acc_data = quality_data["accelerometer"]
                # You could show this in the widget if needed
                
        except Exception as e:
            print(f"Error processing signal quality update: {e}")
    ...
    def start_video_session(self):
        ...
        if self.video_player_window:
            # Connect recalibration signal - ADD THIS
    ...
    def on_new_eeg_prediction(self, prediction_data):
        ...
        if "signal_quality" in prediction_data:
            self._handle_signal_quality_update(prediction_data["signal_quality"])
    ...
    def _restart_calibration(self):
        ...
        # Send stop command to backend first, then restart
        if self.prediction_subscriber:
            print("MeditationPageWidget: Stopping current session before restart...")
            self.prediction_subscriber.stop_session()
            # Wait a bit then restart
            QtCore.QTimer.singleShot(1500, self._restart_session_after_stop)
        else:
            print("MeditationPageWidget: No prediction subscriber available for restart")

    def _restart_session_after_stop(self):
        """Restart session after stop command completes"""
        print(f"MeditationPageWidget: Restarting {self.session_goal} session after stop...")
        
        if not self.prediction_subscriber:
            print("MeditationPageWidget: No prediction subscriber for restart")
            return
            
        if self.session_goal == "RELAXATION":
            success = self.prediction_subscriber.start_relaxation_session()
            print(f"MeditationPageWidget: Relaxation session restart result: {success}")
        elif self.session_goal == "FOCUS":
            success = self.prediction_subscriber.start_focus_session()
            print(f"MeditationPageWidget: Focus session restart result: {success}")
        else:
            print(f"MeditationPageWidget: Unknown session goal for restart: {self.session_goal}")
            
    @QtCore.pyqtSlot(str, dict)
    def on_calibration_status(self, status, baseline_data):
        print(f"Calibration Status: {status}, Data: {baseline_data}")
        ...
        if status == "PAUSED":
            ...
            if self.video_player_window:
                ...
                # The signal quality widget should automatically show the recalibrate button
            ...
            self.is_calibrating = False
            self.is_calibrated = False
            error_msg = baseline_data.get('error_message', 'Calibration failed')
